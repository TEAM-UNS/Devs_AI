...
있고, 이 모듈은 설정을 보고 둘 중 하나를 돌려준다.

★ 로컬 임베딩(BGE-m3)은 여기에 없다. 서비스가 자동으로 돌리는 기능이 아니라
  사람이 수동으로 돌리는 배치라 scripts/embed_local/ 로 분리했다. app 이
  torch(수 GB)를 의존성 그래프에 달고 다니지 않게 하려는 것이다. 태스크·툴은 EmbedderPort
